<x-admin-layout active="products" title="Seus produtos" subtitle="Busque, edite ou publique novos itens.">
    <div class="rounded-2xl border border-[var(--color-primary)]/80 bg-[var(--bg-card)] overflow-hidden shadow-xl shadow-black/15">
        ...
        <div class="p-4 sm:p-5 border-b border-[var(--color-primary)]/80 bg-[var(--bg-card)] flex flex-col md:flex-row gap-4 md:items-end md:justify-between">
            <form action="{{ route('products.search') }}" class="flex-1 min-w-0 flex flex-col sm:flex-row gap-3 sm:items-end" method="POST">
                @csrf
                <div class="flex-1 min-w-0">
                    <label for="product-search" class="block text-xs font-semibold uppercase tracking-wide text-[var(--text-base)] mb-2">Buscar por nome</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-[var(--text-base)] opacity-50">
                            <i data-lucide="search" class="h-4 w-4"></i>
                        </div>
                        <input type="text" id="product-search" name="search"
                            class="w-full rounded-xl border border-[var(--color-primary)] bg-[var(--bg-card)] pl-10 pr-4 py-2.5 text-sm text-[var(--text-base)] placeholder-[var(--text-muted)] outline-none transition-shadow focus:ring-2 focus:ring-[var(--bg-card)]/50"
                ...
                <tbody class="divide-y divide-[var(--color-primary)]/60">
                    @foreach ($products as $product)
                        <tr class="hover:bg-[var(--color-primary)]/10 transition-colors group">
                            ...
                            <td class="px-4 sm:px-6 py-4 text-emerald-600 font-medium tabular-nums">R$ {{ $product->value }}</td>
                            <td class="px-4 sm:px-6 py-4">
