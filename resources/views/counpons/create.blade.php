...
<head>
    ...
    <title>Novo Cupom | ZapCatalog</title>
            ...
            <div class="flex items-center justify-between mb-8 pb-6 border-b border-slate-700/60">
                <div>
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-2">
                        <i data-lucide="plus-circle" class="h-3.5 w-3.5"></i> Cadastro de Desconto
                    </div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Criar Novo Cupom</h1>
                    <p class="text-slate-400 text-sm mt-1">Preencha os dados do cupom de desconto que ficará disponível no sistema.</p>
                </div>
            </div>

            <form action="{{ route('counpons.store') }}" method="post">
                @csrf
